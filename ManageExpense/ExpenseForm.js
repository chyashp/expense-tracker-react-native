...
function ExpenseForm({onCancel,submitButtonLabel, onSubmit, defaultValues}) {
  const [inputs, setInputs ]= useState({
    ...
    date: {value: defaultValues?.date ? getFormattedDate(new Date(defaultValues.date)) : '', isValid: true},
  ...
  function submitHandler(){
    const expenseData = {
        ...
        date: inputs.date.value ? new Date(inputs.date.value) : '',
        description: inputs.description.value
    }

    const amountIsValid = !isNaN(expenseData.amount) && expenseData.amount > 0;
    const dateIsValid = expenseData.date && expenseData.date.toString() !== 'Invalid Date';
